...
- 涂装管理页：国家横条（带计数）+ 载具二级列表 + **涂装包卡片网格**（缩略图 / 半透明名称条 / 拖动排序 / 尺寸与列数自适应）；**导入预览对话框**（导入前可逐个改名、查看告警）
- 涂装包管理：自定义显示名、预览图（选择图片 / 剪贴板 / 清除）、复制（**零字节增量**）、导出恢复原始模组（§3.11）、删除；操作走卡片右键菜单
- 载具管理页：载具列表、显示名映射（自动保存到 `mappings/vehicles.json`）、国家手动归类（`mappings/vehicle_countries.json`，优先于前缀推断）、部件列表
