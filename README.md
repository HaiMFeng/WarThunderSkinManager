...
- 涂装包属性（§3.6）：自定义显示名、预览图（选择图片 / 剪贴板 / 清除）、**逐部件贴图配置**——按 `from` 从同载具各包中相同位置的贴图里挑选（含「不设置」），**部件名右侧显示标签**（最多三个）：**武器 / 导弹**（红色，来自内置武器表 `Assets/units_weaponry.csv`）→ 按命名规律推测的**部位 / 功能**与**贴图类型**（纹理绿 / 法线黄；如「主体」「法线」）；每行另有 **`replace_tex` ⇄ `set_tex` 写入方式滑块**（首次使用弹窗说明，确认后不再提示）；保存为 `packages/<Id>/meta.json` 的 `parts`；据此可「复制包 → 换几个部件」派生新组合
- **激活与同步**（§3.8）：卡片右键**激活为当前涂装**（每个载具同时只激活一套，写入 `loadouts/<载具Id>.json`），**激活即立即同步该载具**（不受自动同步开关影响）；**同步此载具 / 同步全部**按激活包重写 blk 并把贴图调度到 `UserSkins/WTSM/<载具Id>/`（同盘硬链接、跨卷复制、清理旧贴图）；设置里开启自动同步后，改动部件选择按缓冲时间延迟落盘
- 涂装包操作：复制（**零字节增量**，含继承部件配置）、导出恢复原始模组（§3.11）、删除；操作走卡片右键菜单
