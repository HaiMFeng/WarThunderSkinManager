...
│   ├── Assets/units_weaponry.csv     # 内置武器名表（嵌入资源；源文件在 docs/ref/，csproj 指定 LogicalName）
...
- **拖入导入与压缩包导入**（§3.1）：把涂装文件夹或压缩包（可多选）拖到窗口任意位置即导入（拖拽时有提示浮层）；压缩包用 **SharpCompress** 支持 zip / 7z / rar / tar / gzip 等，先解压到 `<资源目录>/imports/extract/` 暂存区再解构，**暂存目录随后自动清理**；**受密码保护的包弹密码框**（可重试 3 次、取消则跳过该包）；预览里可选**「导入成功后删除压缩包」**（默认不勾）；建议包名取压缩包名（包内仅一个顶层文件夹时取该名）
...
- **载具名自动识别**（§3.7）：内嵌「内部标识 → 各语言译名」表（`Assets/units.csv`），取**短名词条 `<标识>_1`**（缺失依次回落全名 `_0` → 标识 → 商店名 `_shop`）；导入新载具时按**界面语言**自动取译名（zh-CN→简体、zh-TW→繁体、其余按语言主标签、缺失回落英文）；解析时清理源表里的**国旗占位符**（`▄`/`◔`/`␗`/私用区等，游戏字库里是小国旗、别处是乱码）与**零宽字符**；**用户自定义名优先**
