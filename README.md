...
- 涂装包属性（§3.6）：自定义显示名、预览图（选择图片 / 剪贴板 / 清除）、**逐部件贴图配置**——按 `from` 从同载具各包中相同位置的贴图里挑选（含「不设置」），**部件名右侧显示标签**（最多三个）：**武器 / 导弹**（红色，来自内置武器表 `Assets/units_weaponry.csv`）→ 按命名规律推测的**部位 / 功能**与**贴图类型**（纹理绿 / 法线黄；如「车体 / 船身」「法线」）；每行另有 **`replace_tex` ⇄ `set_tex` 写入方式滑块**（首次使用弹窗说明，确认后不再提示）；保存为 `packages/<Id>/meta.json` 的 `parts`；据此可「复制包 → 换几个部件」派生新组合
